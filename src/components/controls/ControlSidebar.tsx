...
import type { DisplayTheme, EventOrdering, ModelDefinition } from '@/services/physics/types';
...
const draftFor = (definition: ModelDefinition) => ({ definition, signature: definition.signature, seed: formatRelations(definition.seed), error: '' });

export function ModelPicker() {
  const s = useSimulation();
  return <select className="model-picker" aria-label="Reference model" value={s.definition.id} onChange={event => {
...
export function ControlSidebar() {
  const s = useSimulation();
  const [draft, setDraft] = useState(() => draftFor(s.definition));
  // The editor draft belongs to one definition; a new definition replaces it during render, without an effect.
  if (draft.definition !== s.definition) setDraft(draftFor(s.definition));
  const dirty = draft.signature !== s.definition.signature || draft.seed !== formatRelations(s.definition.seed);
  function apply() {
    try {
      compileRule(draft.signature);
      s.setDefinition({ id: 'custom', name: 'Custom model', group: 'Custom', description: 'A locally defined ordered hypergraph replacement rule.', signature: draft.signature, seed: parseRelations(draft.seed), source: '' });
    } catch (issue) { setDraft({ ...draft, error: issue instanceof Error ? issue.message : 'Invalid model.' }); }
  }
  return <div className="model-panel">
    <div className="section-label"><span>{s.definition.id === 'custom' ? 'Custom model' : s.definition.name}</span></div>
    ...
    <textarea id="rewrite-rule" spellCheck={false} rows={3} value={draft.signature} onChange={e => setDraft({ ...draft, signature: e.target.value })} />
    <div className="field-heading"><label htmlFor="initial-state">Initial relations</label><span>Ordered tuples</span></div>
    <textarea id="initial-state" spellCheck={false} rows={2} value={draft.seed} onChange={e => setDraft({ ...draft, seed: e.target.value })} />
    <p className="input-hint">Each pair of inner braces is one relation. Repeated relations remain distinct occurrences.</p>
    {draft.error && <p className="error-message" role="alert">{draft.error}</p>}
    ...
    <details className="settings" open><summary>Display settings</summary>
